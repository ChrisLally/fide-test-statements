...
with statement_roots_agg as (
  select
    sr.statement_fingerprint,
    array_agg(sr.root order by sr.root) as roots,
    count(*) as root_count
  from statement_roots sr
  group by sr.statement_fingerprint
)
select
  ...
  coalesce(r.roots, array[]::text[]) as roots,
...
left join statement_roots_agg r on r.statement_fingerprint = s.statement_fingerprint
order by s.created_at desc, s.statement_fingerprint asc
limit 20
